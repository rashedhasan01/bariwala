...
public class FloorRepository {
    ...
    private final String CREATE_FLOOR = "{call  BARIOYALA.PKG_GEN_SETUP.CREATE_FLOOR(?,?,?,?,?,?)}";
    private final String UPDATE_FLOOR = "{call  BARIOYALA.PKG_GEN_SETUP.UPDATE_FLOOR(?,?,?,?,?,?,?}";
    private final String DELETE_FLOOR = "{call  BARIOYALA.PKG_GEN_SETUP.DELETE_FLOOR(?,?,?,?)}";
    private final String SEARCH_FLOOR = "{call  BARIOYALA.PKG_GEN_SETUP.GET_FLOOR(?,?,?)}";



    public CreateUserResponse createFloor(CreateFloorRequest request) {
        ...
        try {
            myStatement = connection.prepareCall(CREATE_FLOOR);
            myStatement.setInt("p_PROPERTY_ID", request.getProertyId());
            myStatement.setInt("p_TOTAL_FLAT", request.getNumberOfFlat());
            myStatement.setString("p_FLOOR_TYPE", request.getFloorType());
    ...
    public CreateUserResponse updateFloor(UpdateFloorRequest request) {
        ...
        try {
            myStatement = connection.prepareCall(UPDATE_FLOOR);
            myStatement.setInt("p_FLOOR_ID", request.getFloorID());
            myStatement.setInt("p_TOTAL_FLAT", request.getNumberOfFlat());
            myStatement.setString("p_FLOOR_TYPE", request.getFloorType());
            myStatement.setString("p_SHOWING_ORDER", "1");
    ...
    public CreateUserResponse deleteFloor(DeleteFloorRequest request) {
        ...
        if(request.getStatus().toUpperCase().equals("D")) {
            ...
            try {
                ...
                myStatement = connection.prepareCall(DELETE_FLOOR);
                myStatement.setInt("p_FLOOR_ID", request.getFloorID());
    ...
    public SearchPropertyResponse searchFloor(SearchPropertyRequest request) {
        ...
        try {
            myStatement = connection.prepareCall(SEARCH_FLOOR);
